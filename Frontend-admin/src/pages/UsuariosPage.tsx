                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <button
                            ...
                            className="font-body font-semibold text-primary hover:underline text-left"
                          >
                            {u.nombre}
                          </button>
                          <p className="font-body text-xs text-on-surface-variant">
                            ID: {u.id} · Registrado: {u.created_at ? new Date(u.created_at).toLocaleDateString('es-AR') : '-'}
                          </p>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {u.roles.length === 0 ? (
                          ...
                          u.roles.map((rol) => (
                            <span
                              key={rol}
                              className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${roleColors[rol] ?? 'bg-gray-100 text-gray-700'}`}
                            >
                              {roleLabels[rol] ?? rol}
                            </span>
                          ))
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`transition-colors p-1 ${u.activo ? 'text-error hover:text-red-400' : 'text-green-600 hover:text-green-400'}`}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-surface-container-high rounded-lg p-4 col-span-2">
                ...
                <p className="font-body text-sm text-on-surface">
                  {selectedUser.created_at ? new Date(selectedUser.created_at).toLocaleString('es-AR') : '-'}
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedUser.roles.length === 0 ? (
                  ...
                  selectedUser.roles.map((rol) => (
                    <span
                      key={rol}
                      className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold ${roleColors[rol] ?? 'bg-gray-100 text-gray-700'}`}
                    >
                      {roleLabels[rol] ?? rol}
                    </span>
                  ))
      ...
      <Modal isOpen={roleModalOpen} onClose={() => setRoleModalOpen(false)} title="Editar Roles">
        {selectedUser && (
          <div className="space-y-4">
            ...
            <p className="font-body text-sm text-on-surface-variant">
              Seleccioná los roles que querés asignarle a este usuario:
            </p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${
                          roleColors[rol.codigo] ?? 'bg-gray-100 text-gray-700'
                        }`}
                      >
                    ...
                    <div className="flex items-center gap-2">
                      {tieneRol ? (
                        <button
                          onClick={() => removerRol.mutate({ userId: selectedUser.id, rolCodigo: rol.codigo })}
                        ...
                        <button
                          onClick={() => asignarRol.mutate({ userId: selectedUser.id, rolCodigo: rol.codigo })}
                          className="flex items-center gap-1 px-3 py-1.5 bg-primary-container/20 text-primary rounded-lg text-xs font-semibold hover:bg-primary-container/30 transition-colors"
